...
module generate(part=part){
	...
	if (part == 7) gear7();
	if (part == 8) gear8();
	if (part == 9) rotate([0,90-theta,0])center();
	if (part == 10) {pin1();translate([12,0,0])pin2();}
	if (part == 11) complete1();
}

generate();

module shape(){
	//%import_stl("earthmap.stl", convexity=2);
	scale(1.1) import("earthmap.stl", convexity=2);
}
...
hz=0;// heart z-position (motion relative to 100mm original height)
...
module complete(){
intersection(){
scale(hs/100)translate([0,0,hz]) shape();
